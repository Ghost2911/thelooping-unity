...
public class SummonStatus : Status
{
    ...
    public override void Tick()
    {
        ...
        for (int i = 0; i < maxUnitCount; i++)
        {
            ...
            else
            {
                ...
                {
                    if (enemy != null)
                        summonUnits[i].SetTarget(enemy.GetComponent<EntityStats>());
                    else
    ...
    {
        for (int i = 0; i < maxUnitCount; i++)
            if (summonUnits[i] != null) summonUnits[i].stats.Damage(new HitInfo(statusData.damageType, 999, 0, Vector3.zero, Color.white,null,true));
    }
