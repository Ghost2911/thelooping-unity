...
public class PlayerInput : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.Delete))
            stats.Damage(new HitInfo(DamageType.Impact, 999,0,Vector3.zero,Color.cyan));
    }
    ...
    private void Attack()
    {
        ...
        //ParticlePool.instance.GetParticle(transform.position + stats.direction*2f);
        Statistic.instance.psyhotypeData.AddPoints(Psyhotype.Killer,10);
        Statistic.instance.psyhotypeData.PrintPoints();
        ...
        foreach (Collider enemy in hitEnemies)
        {
            if (enemy.transform != transform)
            {
                ...
                damagable?.Damage(new HitInfo(stats.damageType,(int)(stats.attack * stats.attackMultiplier), stats.attackKnockback, stats.direction, Color.red, stats));
    ...
    public void FlipButtonClick() { if (!stats.isStunned) if (!stats.animator.GetCurrentAnimatorStateInfo(0).IsName("Flip")) { stats.animator.SetTrigger("Flip"); StartCoroutine(Flip()); } }
